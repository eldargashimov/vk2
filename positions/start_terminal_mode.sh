...
# Запуск всех станций в терминальном режиме

terminal="gnome-terminal --execute"

echo -e "\nЗапуск системы - терминальный режим"
...
do
	...
	then
		...
		cd rls$i
		# Запускаем
		$terminal "`pwd`/rls$i.sh" > /dev/null
		echo "ОК"
		cd ..
	else
		echo "уже запущена"
...
do
	...
	then
		...
		cd zrdn$i
		# Запускаем
		$terminal "`pwd`/zrdn$i.sh" > /dev/null
		echo "ОК"
		cd ..
	else
		echo "уже запущен"
