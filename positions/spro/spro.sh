...
trap "echo 'Конец работы'; logline 'Конец работы'; sendmsg 'kp' 'Конец работы'; exit" SIGINT
...
#			echo -en "tgt accidentally died\n"
			...
			then
				...
				# однако СПРО видит не все типы целей	
				...
				# СПРО видит только ББ БР
				if [ "$tgt_type" = "icbm" ]
				then
					# Цель входит в зону ответственности позиции,
					# т.е. на данном этапе СПРО видит цель
	...
	then
		...
		echo -e "\e[3m(нет видимых целей)\e[0m"
	fi
	...
	then
		# Выстрелов не было
		echo -e "\n\e[3m(не было выстрелов за последний такт)\e[0m"
	fi
